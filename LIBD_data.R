...
slice <- c("151507", "151508", "151509", "151510", "151669", "151670", "151671", "151672", "151673", "151674", "151675", "151676")
n.clusters <- c(7, 7, 7, 7, 5, 5, 5, 5, 7, 7, 7, 7)
...
for (k in 1:12) {
  for (i in c) {
    ...
    # )
    # 
    # saveRDS(spaData, file = paste0("spaData-", sample.id, ".rds"))
    sample.id <- "151673"
    ...
    #   # is.spike <- base::grepl("^ERCC", rowData(spaData)$gene_name, ignore.case = TRUE)
    #   is.mito <- base::grepl("^mt-", rowData(spaData)$gene_name, ignore.case = TRUE)
    #   spaData <- addPerCellQCMetrics(spaData, subsets=list(Mito=is.mito))
    #   summary(colData(spaData)$subsets_Mito_percent)
    #   per.cell <- perCellQCMetrics(spaData, subsets=list(Mito=is.mito))
    #   discarded <- perCellQCFilters(per.cell, nmads = 4.5)
    #   colSums(as.matrix(discarded))
    #   spaData$discard <- discarded$discard
    #
    #   # not filter spots based on mito
    #   gridExtra::grid.arrange(
    #     plotColData(spaData, y="sum", colour_by="discard") +
    #       scale_y_log10() + ggtitle("Total count"),
    #     plotColData(spaData, y="detected", colour_by="discard") +
    #       scale_y_log10() + ggtitle("Detected features"),
    #     plotColData(spaData, y="subsets_Mito_percent", colour_by="discard") +
    #       ggtitle("Mito percent"),
    #     ncol=1
    #   )
    #   # to confirm that there are no cells with both large total counts and large mitochondrial counts,
    ...
    #   plotColData(spaData, x="sum", y="subsets_Mito_percent", colour_by="discard")
    #
    #   # filter out spots first then mito genes, then less expressed
    #   #
    #   # spaData <- spaData[,!discarded$discard]
    
    per.cell <- perCellQCMetrics(spaData)
    per.gene <- perFeatureQCMetrics(spaData)
    libsize.drop <-
      isOutlier(per.cell$total,
                nmads = 3,
                type = "lower",
                log = TRUE)
    ...
    spaData <-
      computeSumFactors(spaData, cluster = cluster, min.mean = 0.1)
    ...
    adj.2d <-
      calculate.adj.matrix(x = x_array,
                           y = y_array,
                           histology = FALSE)
    refined.pred <-
      refine(
    ...
    pdf(file = paste0("./PCA/1set_pred_", seed, ".pdf"))
    print(plotSpots(
      spaData,
      annotate = "spa.y_pred",
      palette = colors,
    ...
    colors <- plot_color[1:num_cluster]
    pdf(file = paste0("./PCA/1set_refined.pred_", seed, ".pdf"))
    print(plotSpots(
      spaData,
